...
func TestServer(t *testing.T) {
	...
	assert.Equal(t, (*models.Commitment)(nil), respAttestation.Commitment)
	...
	assert.Equal(t, *bestblockhash, server.latestAttestation.Commitment.GetCommitmentHash())

	// Test latest attestation again after update
	respAttestation, errAttestation = server.GetLatestAttestation()
	assert.Equal(t, nil, errAttestation)
	assert.Equal(t, *txid, respAttestation.Txid)
	assert.Equal(t, *bestblockhash, respAttestation.Commitment.GetCommitmentHash())
}
